...
class sesiones
{
	...
	/**
	 *@method sesiones Verifica la existencia de una sesion válida en la máquina del cliente
	...
	function sesion($configuracion)
		...
		else 
		{
			...
			$cadena_sql = "UPDATE ".$configuracion["prefijo"]."valor_sesion SET valor=".(time()+$this->expiracion)." WHERE id_sesion='".($this->sesion_id)."' AND variable='expiracion'";
			//echo $cadena_sql;
	...
	function numero_sesion()
		...
		{
			if(isset($_GET["aplicativo"]))
			{
				$this->id_sesion=$_GET["aplicativo"];
			}
			else
			{
				$this->id_sesion='0';
...
/**
	...
	function crear_sesion($configuracion,$usuario_aplicativo,$nivel_acceso)
		...
		else 
		{
			...
			$tiempo=time()+$configuracion["expiracion"];
			//echo $tiempo;
			//echo "La expiracion de las cookies es: ".$this->expiracion."<br>";
			setcookie("aplicativo",$this->sesion_id,(time()+$configuracion["expiracion"]),"/");
			...
			{
				//Insertar expiracion
				$this->resultado=$this->guardar_valor_sesion($configuracion,'expiracion',(time()+$configuracion["expiracion"]),$this->sesion_id);
				if($this->resultado)
				{
    ...
    function rescatar_valor_sesion($configuracion,$variable)
    {
		...
		// Busca la sesión
		$this->cadena_sql = "SELECT valor FROM ".$configuracion["prefijo"]."valor_sesion WHERE id_sesion = '".($this->sesion_id)."'  AND variable='".$variable."'";
...
/**
	...
	$this->tiempo=time();
	$cadena_sql = "SELECT id_sesion,valor FROM ".$configuracion["prefijo"]."valor_sesion WHERE variable='expiracion'";
	$db_sel = new dbms($this->aplicativo);
	$db_sel->especificar_enlace($this->conexion_id);
	...
	if($conteo>0)
	{
		...
		for($i=0;$i<$conteo;$i++)
		{
			...
			{
				$this->sql = "DELETE FROM ".$configuracion["prefijo"]."valor_sesion WHERE id_sesion='".$registro[$i][0]."'";
				//echo $this->sql."<br>";
...
/**
	...
	function autenticar($id_conexion)
	{
		...
		if (is_resource($id_conexion)) 
		{
			...
			$this->cadena_sql="SELECT usuario,clave FROM ".$configuracion["prefijo"]."usuario WHERE usuario='".$this->usuario_aplicativo."' AND clave='".$this->clave_aplicativo."'";
			$db_sel = new dbms();
