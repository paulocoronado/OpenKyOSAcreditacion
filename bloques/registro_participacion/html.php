...
if(isset($_GET['id_participacion']))
{
	...
	{
		$cadena_sql="SELECT `nombre` , `id_tipo_participacion` , `anno`,`identificacion` ";
		$cadena_sql.="FROM `".$configuracion["prefijo"]."profesor_info_participacion` ";
		$cadena_sql.="WHERE id_participacion=".$_GET['id_participacion']." LIMIT 1";
		//echo $cadena_sql;
  ...
  <table class="bloquelateral" cellpadding="5" cellspacing="1" width="100%">
    <tbody>
      <tr class="mensajealertaencabezado">
		...
		Informaci&oacute;n de participaciones en programas
	</td>
      </td>
      </tr>	
      <tr class="bloquecentralcuerpo">
        ...
        <td class="celdatabla">
		<input maxlength="80" size="30" name="nombre" value="<?PHP   echo $registro[0][0] ?>">

	</td>
      </tr>
      <tr class="bloquecentralcuerpo">
        <td class="celdatabla">
		Tipo de participaci&oacute;n:
	</td>
        <td class="celdatabla"><?PHP  
include_once($configuracion["raiz_documento"].$configuracion["clases"]."/html.class.php");
$html=new html();
$busqueda="SELECT id_valor,nombre_corto FROM ".$configuracion["prefijo"]."valores_profesor WHERE id_tipo_valor=5 ORDER BY nombre_corto";
$mi_cuadro=$html->cuadro_lista($busqueda,'id_tipo_participacion',$configuracion,$registro[0][1],0,0);
echo $mi_cuadro;
	?></td>
      </tr>
      <tr align="center">
        <td colspan="2" rowspan="1" valign="undefined">
		<input type="hidden" name="action" value="registro_participacion">
		<input type="hidden" name="anno" value="<?PHP   echo $_GET["anno"]?>">
		<input type="hidden" name="id_participacion" value="<?PHP   echo $_GET["id_participacion"]?>">
		<input type="hidden" name="identificacion" value="<?PHP   echo $registro[0][3]?>">
  ...
  <table class="bloquelateral" cellpadding="5" cellspacing="1" width="100%">
    <tbody>
      ...
      <tr class="bloquecentralcuerpo">
        ...
        <td class="celdatabla">
		<input maxlength="80" size="30" name="nombre">
...
$html=new html();
$busqueda="SELECT id_valor,nombre_corto FROM ".$configuracion["prefijo"]."valores_profesor WHERE id_tipo_valor=5 ORDER BY nombre_corto";
$mi_cuadro=$html->cuadro_lista($busqueda,'id_tipo_participacion',$configuracion,-1,0,0);
echo $mi_cuadro;
	?></td>
      </tr>
      <tr class="bloquecentralcuerpo">
