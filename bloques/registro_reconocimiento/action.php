...
{	
	...
	{
		$this->cadena_sql='SELECT * FROM aplicativo_profesor_info_reconocimiento WHERE id_reconocimiento='.$_POST['id_reconocimiento'];
		...
		if($this->campos>0)
		{
			...
			$this->cadena_sql = "UPDATE `".$configuracion["prefijo"]."profesor_info_reconocimiento` SET ";
			$this->cadena_sql.= "nombre='".$_POST['nombre']."',";
			$this->cadena_sql.= "puntos=".$_POST['puntos'];
			$this->cadena_sql.= " WHERE id_reconocimiento=".$_POST['id_reconocimiento'];
			echo $this->cadena_sql.'<br>';
			
		}
		else
		{
			$this->cadena_sql = "INSERT INTO ".$configuracion["prefijo"]."profesor_info_reconocimiento ";
			$this->cadena_sql.= "( `id_reconocimiento` ,`identificacion` , `nombre` , `puntos`,`anno` ) ";					
			$this->cadena_sql.= "VALUES (";
			$this->cadena_sql.= "NULL, '".$_POST['identificacion']."','".$_POST['nombre']."',".$_POST['puntos'].",".$_POST['anno'];
			$this->cadena_sql.=")";
			//echo $this->cadena_sql.'<br>';
		}
		//echo $this->cadena_sql."<br>";
	...
	{	
		$this->cadena_sql='SELECT * FROM aplicativo_profesor_info_reconocimiento WHERE nombre="'.$_POST['nombre'].'" AND anno='.$_POST['anno'];
		//echo $this->cadena_sql.'<br>';
		$this->acceso_db->registro_db($this->cadena_sql,0);
		$this->registro=$this->acceso_db->obtener_registro_db();
		$this->campos=$this->acceso_db->obtener_conteo_db();
		if($this->campos>0)
			{
				...
				$this->cadena_sql = "UPDATE `".$configuracion["prefijo"]."profesor_info_reconocimiento` SET ";
				$this->cadena_sql.= "nombre='".$_POST['nombre']."',";
				$this->cadena_sql.= "puntos=".$_POST['id_tipo_reconocimiento'];
				$this->cadena_sql.= " WHERE id_reconocimiento=".$_POST['id_reconocimiento'];
				//echo $this->cadena_sql.'<br>';
