...
* // Control de botones revisado y corregido "alejandro camacho" 29-05-2008
*****************************************************************************************************************/
?><?php
	...
	if(isset($_GET['accion']))
	{
		...
		{	
			...
			case '1':
				...
				$cadena_hoja="SELECT id_instrumento,nombre,fecha_creacion, comentario,id_usuario,estado";
				$cadena_hoja.=" FROM ".$configuracion["prefijo"]."instrumento ";
				$cadena_hoja.=" WHERE id_usuario=".$el_usuario;
				$cadena_hoja.=" ORDER BY nombre ";
				
				$cadena_sql="SELECT id_instrumento,nombre,fecha_creacion, comentario,id_usuario,estado";
				$cadena_sql.=" FROM ".$configuracion["prefijo"]."instrumento ";
			...
			default:
				...
				$cadena_hoja.=" FROM ".$configuracion["prefijo"]."instrumento ";
				$cadena_hoja.=" WHERE id_usuario=".$el_usuario;
				$cadena_hoja.=" AND (tipo=0 OR tipo=1)";
...
<?php
			if($el_usuario==$registro[$contador][4])
			{
				...
				$cadena_sql.="* ";
				$cadena_sql.="FROM ";
				$cadena_sql.=$configuracion["prefijo"]."a_instrumento ";
				$cadena_sql.="WHERE ";
				$cadena_sql.="id_instrumento=".$registro[$contador][0]." ";
				$cadena_sql.="LIMIT 1";
				//echo $cadena_sql;
				$acceso_db->registro_db($cadena_sql,0);
...
<a href="<?php
	$opcion=$configuracion["site"].'/index.php?page='.enlace('borrar_formulario');
	$opcion.=$variable; 
	$opcion.="&opcion=formulario";
...
<tr class="bloquecentralcuerpo">
	...
	$opcion=$configuracion["site"].'/index.php?page='.enlace('comite_formulario');
	$opcion.=$variable;
