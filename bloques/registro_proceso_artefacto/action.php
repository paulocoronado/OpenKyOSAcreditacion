...
if($enlace)
{
	...
	if(isset($_POST['asociar']) || isset($_POST['actualizar']))
	{
		...
		$cadena_sql_1="INSERT INTO ".$configuracion["prefijo"]."p_borrador (id_elemento,id_sesion,estado) VALUES ";
			...
			if(isset($_POST['asociar']))
			{
				if(substr($clave,0,6)=="ORDEN_")
				{
					...
					if(isset($_POST["ELEMENTO_".$pregunta]))
					{
						...
						$cadena_sql_1.="(".$pregunta.",'".$esta_sesion."',1)";
						$concatenar_2++;
		...
		if($resultado==TRUE)
		{
			...
			echo "<script>location.replace('".$configuracion["host"].$configuracion["site"]."/index.php?page=".enlace('registro_proceso_artefacto').$variable."')</script>";   
	...
	{	
		...
		if($resultado==TRUE)
		{
			...
			unset($_POST["action"]);
			include_once($configuracion["raiz_documento"].$configuracion["bloques"]."/enlace.inc.php");
			echo "<script>location.replace('".$configuracion["host"].$configuracion["site"]."/index.php?page=".enlace('agregar_proceso')."')</script>";   
