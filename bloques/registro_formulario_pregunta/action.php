...
if($enlace)
{
	...
	if(isset($_POST['asociar']) || isset($_POST['actualizar']))
	{
		...
		if($resultado==TRUE)
		{
			...
			echo "<script>location.replace('".$configuracion["host"].$configuracion["site"]."/index.php?page=".enlace('registro_formulario_pregunta').$variable."')</script>";   
	...
	else
	{
		...
		unset($_POST["action"]);
		include_once($configuracion["raiz_documento"].$configuracion["bloques"]."/enlace.inc.php");
		echo "<script>location.replace('".$configuracion["host"].$configuracion["site"]."/index.php?page=".enlace('agregar_formulario')."')</script>";   
