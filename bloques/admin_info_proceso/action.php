	...
	else
	{
		...
		*/
		...
		if (is_resource($enlace))
		{
			...
			$cadena_sql="DELETE FROM aplicativo_proceso_borrador WHERE id_sesion='".$esta_sesion."'";
			echo $cadena_sql."<br>";
			$acceso_db->ejecutar_acceso_db($cadena_sql);
			$cadena_sql="DELETE FROM aplicativo_p_borrador WHERE id_sesion='".$esta_sesion."'";
			//echo $cadena_sql."<br>";
			$acceso_db->ejecutar_acceso_db($cadena_sql);
			/*Esto se realiza para poder mostrar los valores del artefacto antes de guardarlo en la base de datos */
			$cadena_sql="INSERT INTO aplicativo_proceso_borrador ";
			$cadena_sql.="(";
